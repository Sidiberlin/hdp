...
## [5.1.9-QoL6] - 2026-09-24

### Added
- **Periodic re-ingestion**: new `ingest-scheduler` sidecar re-indexes edited/added wiki pages every `HDP_INGEST_INTERVAL_MIN` minutes (default 5, 0 disables) via the incremental `--missing-only` pass; flock-guarded, backoff on errors, preflight skips when the index is empty (no surprise bulk ingests). Ships via compose bind-mount - no image rebuild needed.
- **OpenAI-style ingestion API**: `POST /v1/ingest/pages` (bearer `HDP_INGEST_API_KEY`, generated by the installer) accepts `{category, pages:[{title, content}]}`, writes pages into the wiki category via a dedicated `HDPIngestBot` account (admin-credential fallback for upgraded installs), indexes them immediately; OpenAI-style error envelopes; 401/403/fail-closed when unconfigured.
- Docs: scheduler + API reference pages in `docs/wiki/modules/` and README-DOCKER.

### Fixed
- **Citations**: section metadata now forwarded end-to-end - citation labels are distinct per section (`Page - Section`) and links carry real MediaWiki anchors (click-through verified in a real browser; sticky-header offset handled).
- **GPU installs silently ran on CPU**: CUDA wheel tag default moved cu124->cu126 (current torch), ARG plumbing fixed so the index selection actually applies, and the haystack entrypoint now **fails loudly** when `HAYSTACK_DEVICE=gpu` but CUDA is unavailable instead of silently serving CPU.
- Update flow: `ingest-scheduler` included in the post-update service summary; port-migration warning, wgServer reconciliation, threaded chatbot proxy (from QoL5 line) unchanged.
